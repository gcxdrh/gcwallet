...
// zgc: optional setting to unlock wallet for block minting only;
...
// zgc: total coins staked (non-spendable until maturity)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                if (pcoin->nTime > nSpendTime)
                    continue;  // zgc: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // zgc: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // zgc: not avatar mode
                    {
...
// zgc: create coin stake transaction
...
// zgc: check 'spent' consistency between wallet and txindex
// zgc: fix wallet spent state according to txindex
...
// zgc: disable transaction (only for coinstake)
