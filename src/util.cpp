...
// Copyright (c) 2011-2012 The GC developers
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows: C:\Documents and Settings\username\Application Data\GCOIND
    // Mac: ~/Library/Application Support/GCOIND
    // Unix: ~/.zgcoind
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "GCOIND";
#else
    fs::path pathRet;
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong GC will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("GC"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "GC:" << FormatVersion(GC_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) / "GC.lnk";
}
...
{
    return GetAutostartDir() / "zgc.desktop";
}
